...
namespace Server.Items
{
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable
	{
		...
		public double FocusCoeff { get { return m_FocusCoeff; } set { m_FocusCoeff = value; } }
		...
		public override void OnRemoved( object parent )
		{
			if ( parent is Mobile )
			{
				...
				{
					Spell spell = FocusAttack.GetSpell( m );
					...
					FocusAttack.RemoveBonus( this );

					FocusAttack.m_Table2.Remove( m );
				}
		...
		public void Perfection( Mobile attacker, Mobile defender, bool raise )
		{
			PlayerMobile pm = attacker as PlayerMobile;

			if ( pm == null || defender == null )
			{
				return;
			}

			if ( pm.HonorOpponent != null && pm.HonorOpponent == defender && pm.Skills[ SkillName.Bushido ].Base >= 50 )
			{
				if ( raise )
				{
					if ( pm.Perfection < 10 )
					{
						if ( pm.Perfection < 9 )
						{
							pm.SendLocalizedMessage( 1063255 ); // You gain in Perfection as you precisely strike your opponent.
						} 
						else
						{
							pm.SendLocalizedMessage( 1063254 ); // You have Achieved Perfection in inflicting damage to this opponent!
						}
				...
				else
				{
					if ( pm.Perfection > 3 )
					{
						pm.SendLocalizedMessage( 1063257 ); // You have lost some Perfection in fighting this opponent.

						pm.Perfection -= 3;
					}
					else
					{
						pm.SendLocalizedMessage( 1063256 ); // You have lost all Perfection in fighting this opponent.
		...
		public virtual TimeSpan OnSwing( Mobile attacker, Mobile defender )
		{
			...
			if ( canSwing && attacker.HarmfulCheck( defender ) )
			{
				...
				{
					Perfection( attacker, defender, true );

					OnHit( attacker, defender );
				}
				else
				{
					Perfection( attacker, defender, false );
		...
		public virtual int AbsorbDamageAOS( Mobile attacker, Mobile defender, int damage )
		{
			...
			if ( defender.Player || defender.Body.IsHuman )
			{
				...
				double dexmodifier = (80 - defender.Dex)/100;

				if ( shield != null )
				{
					double chance = (defender.Skills[ SkillName.Parry ].Value - defender.Skills[ SkillName.Bushido ].Value)/400.0;
					...
					if ( defender.Skills[ SkillName.Parry ].Value >= 100.0 || defender.Skills[ SkillName.Bushido ].Value >= 100.0 )
					{
						chance += 0.05;
					}

					if ( Evasion.UnderEffect( defender ) )
					{
						chance += chance*0.5;
					}

					chance *= (1 - dexmodifier);
				...
				else if ( !(defender.Weapon is Fists) && !(defender.Weapon is BaseRanged) )
				{
					...
					double chance = 0, newchance = 0, legacy = 0;

					if ( weapon.Layer == Layer.OneHanded )
					{
						newchance = (defender.Skills[ SkillName.Parry ].Value*defender.Skills[ SkillName.Bushido ].Value)/48000;

						legacy = (defender.Skills[ SkillName.Parry ].Value)/800;

						chance = Math.Max( newchance, legacy );
					...
					{
						newchance = (defender.Skills[ SkillName.Parry ].Value*defender.Skills[ SkillName.Bushido ].Value)/41140;
						...
						defender.CheckSkill( SkillName.Parry, chance );
					}
				...
				if ( blocked )
				{
					if ( CounterAttack.UnderEffect( defender ) )
					{
						AOS.Damage( attacker, defender, damage, 100, 0, 0, 0, 0 );

						defender.Send( new SetNewSpell( 0x94, 0 ) );

						CounterAttack.m_Table.Remove( defender );
					}

					defender.FixedEffect( 0x37B9, 10, 16 );
					damage = 0;

					if ( Confidence.UnderEffect( defender ) )
					{
						defender.SendLocalizedMessage( 1063117 ); // Your confidence reassures you as you successfully block your opponent's blow.

						Skill skill = defender.Skills[ SkillName.Bushido ];

						double v = skill.Value;

						int hits = Utility.Random( 1, (int) (v/12.0) );

						int stam = Utility.Random( 1, (int) (v/5.0) );
		...
		#region SE Spells and special moves
		public static Hashtable m_TableStealth = new Hashtable();

		public static Hashtable m_TableSurprise = new Hashtable();

		public static Hashtable m_TableDeathStrike = new Hashtable();

		public static Hashtable m_TableCheckAlive = new Hashtable();

		public static Hashtable m_TableSwing = new Hashtable();

		public static Hashtable m_TableFail = new Hashtable();

		public static bool UnderStealth( Mobile m )
		{
			return m_TableStealth.Contains( m );
		}

		public static bool UnderSurprise( Mobile m )
		{
			return m_TableSurprise.Contains( m );
		}

		public static bool UnderDeathStrike( Mobile m )
		{
			return m_TableDeathStrike.Contains( m );
		}

		public static bool UnderSwing( Mobile m )
		{
			return m_TableSwing.Contains( m );
		}

		public static bool UnderFail( Mobile m )
		{
			return m_TableFail.Contains( m );
		}

		private void Expire_CallbackStealth( object state )
		{
			Mobile m = (Mobile) state;

			m_TableStealth.Remove( m );
		}

		private void Expire_CallbackSurprise( object state )
		{
			Mobile m = (Mobile) state;

			m_TableSurprise.Remove( m );
		}

		private void Expire_CallbackDeathStrike( object state )
		{
			Mobile m = (Mobile) state;

			Mobile from = this.Parent as Mobile;
			...
			if ( from != null )
			{
				...
				if ( m.CalculateSteps )
				{
					damage = m.GetDeathStrikeBonus( from.Skills[ SkillName.Ninjitsu ].Value, false );

					m.Damage( (int) damage, from );

					m.CalculateSteps = false;

					m.Steps = 0;

					// OSI now don't reset this counter, but Mr.Tact promised that it will be fixed
					m.Attacker.Tracking_Table.Remove( m );

					m.Attacker = null;
				}
			}

			m_TableDeathStrike.Remove( m );
		}

		private void Expire_CallbackCheckAlive( object state )
		{
			Mobile m = (Mobile) state;
			...
			if ( from != null )
			{
				if ( !m.Alive )
				{
					from.FixedParticles( 0x373A, 1, 17, 2018, EffectLayer.Waist );

					from.Hits += Utility.Random( 30, 40 );

					Timer t = (Timer) m_TableSwing[ from ];
					...
					m_TableSwing[ from ] = t = Timer.DelayCall( TimeSpan.FromSeconds( 20.0 ), new TimerStateCallback( Expire_CallbackSwing ), from );
				}
				else
				{
					from.FixedParticles( 0x37C4, 1, 8, 0x26B8, 0, 0, EffectLayer.Waist );

					object[] mods = new object[6] {new DefaultSkillMod( SkillName.MagicResist, true, -(int) from.Skills[ SkillName.MagicResist ].Value ), new ResistanceMod( ResistanceType.Physical, -40 ), new ResistanceMod( ResistanceType.Fire, -40 ), new ResistanceMod( ResistanceType.Cold, -40 ), new ResistanceMod( ResistanceType.Poison, -40 ), new ResistanceMod( ResistanceType.Energy, -40 )};

					from.AddSkillMod( (SkillMod) mods[ 0 ] );

					for ( int i = 0; i < 5; i++ )
					{
						from.AddResistanceMod( (ResistanceMod) mods[ i + 1 ] );
					}

					m_TableFail[ from ] = true;

					Timer fail = new FailTimer( from, mods );

					fail.Start();
				}
			}

			m_TableCheckAlive.Remove( m );
		}

		private class FailTimer : Timer
		{
			private Mobile m;
			private object[] mod;

			public FailTimer( Mobile mobile, object[] mods ) : base( TimeSpan.FromSeconds( 6.0 ) )
			{
				m = mobile;
			...
			protected override void OnTick()
			{
				m.RemoveSkillMod( (SkillMod) mod[ 0 ] );

				for ( int i = 0; i < 5; i++ )
				{
					m.RemoveResistanceMod( (ResistanceMod) mod[ i + 1 ] );
				}

				m_TableFail.Remove( m );

				Stop();
			}
		}

		private void Expire_CallbackSwing( object state )
		{
			Mobile m = (Mobile) state;

			m_TableSwing.Remove( m );
		}
		#endregion
		...
		public virtual void OnHit( Mobile attacker, Mobile defender )
		{
			...
			if ( ability != null && ability is TalonStrike )
			{
			...
			if ( HonorableExecution.UnderEffect( attacker ) )
			{
				Spell spell = HonorableExecution.GetSpell( attacker );

				if ( spell.CheckSequence() )
				{
					Timer t = (Timer) m_TableCheckAlive[ defender ];
					...
					m_TableCheckAlive[ defender ] = t = Timer.DelayCall( TimeSpan.FromSeconds( 0.001 ), new TimerStateCallback( Expire_CallbackCheckAlive ), defender );
				}

				attacker.Send( new SetNewSpell( 0x91, 0 ) );

				HonorableExecution.m_Table.Remove( attacker );

				HonorableExecution.m_Table2.Remove( attacker );

				spell.FinishSequence();
			}

			if ( Confidence.UnderEffect2( defender ) )
			{
				Timer t = Confidence.GetTimer( defender );
				...
				Confidence.m_Table2.Remove( defender );
			}

			if ( LightningStrike.UnderEffect( attacker ) )
			{
				Spell spell = LightningStrike.GetSpell( attacker );
				...
				if ( spell.CheckSequence() )
				{
					...
					double chance = (value*value)/72000;
				...
				attacker.Send( new SetNewSpell( 0x95, 0 ) );

				LightningStrike.m_Table.Remove( attacker );

				LightningStrike.m_Table2.Remove( attacker );

				spell.FinishSequence();
			}

			if ( MomentumStrike.UnderEffect( attacker ) )
			{
				Spell spell = MomentumStrike.GetSpell( attacker );

				if ( spell.CheckSequence() )
				{
					Mobile enemy = null;

					foreach ( Mobile m in attacker.GetMobilesInRange( 6 ) )
					{
						if ( m != defender && m.Combatant != null && m.Combatant == attacker )
						{
							enemy = m;
						}
					}
					...
					if ( enemy != null )
					{
						AOS.Damage( enemy, attacker, damage*2, 100, 0, 0, 0, 0 );

						attacker.SendLocalizedMessage( 1063171 ); // You transfer the momentum of your weapon into another enemy!

						enemy.SendLocalizedMessage( 1063172 ); // You were hit by the momentum of a Samurai's weapon!

						enemy.FixedParticles( 0x37B9, 1, 4, 0x251D, 0, 0, EffectLayer.Waist );
					}
					else
					{
				...
				attacker.Send( new SetNewSpell( 0x96, 0 ) );

				MomentumStrike.m_Table.Remove( attacker );

				MomentumStrike.m_Table2.Remove( attacker );

				spell.FinishSequence();
			}

			if ( DeathStrike.UnderEffect( attacker ) )
			{
				Spell spell = DeathStrike.GetSpell( attacker );

				if ( spell.CheckSequence() )
				{
					if ( !UnderDeathStrike( defender ) )
					{
						...
						defender.FixedParticles( 0x374A, 1, 17, 5038, EffectLayer.Waist );

						defender.Steps = 0;

						defender.CalculateSteps = true;

						defender.Attacker = attacker;

						Timer t = (Timer) m_TableDeathStrike[ defender ];
						...
						m_TableDeathStrike[ defender ] = t = Timer.DelayCall( TimeSpan.FromSeconds( 5.0 ), new TimerStateCallback( Expire_CallbackDeathStrike ), defender );
					}
					else
					{
				...
				attacker.Send( new SetNewSpell( 0xF6, 0 ) );

				DeathStrike.m_Table.Remove( attacker );

				DeathStrike.m_Table2.Remove( attacker );

				spell.FinishSequence();
			}

			if ( SurpriseAttack.UnderEffect( attacker ) )
			{
				Spell spell = SurpriseAttack.GetSpell( attacker );

				if ( spell.CheckSequence() )
				{
					if ( SurpriseAttack.UnderEffect( attacker ) && !attacker.Hidden )
					{
						attacker.SendLocalizedMessage( 1063087 ); // You must be in stealth mode to use this ability.					

						attacker.RevealingAction();

						attacker.Send( new SetNewSpell( 0xF9, 0 ) );

						SurpriseAttack.m_Table.Remove( attacker );

						SurpriseAttack.m_Table2.Remove( attacker );
					...
					Timer t = (Timer) m_TableSurprise[ attacker ];
					...
					m_TableSurprise[ attacker ] = t = Timer.DelayCall( TimeSpan.FromSeconds( 6.0 ), new TimerStateCallback( Expire_CallbackSurprise ), attacker );

					Timer t1 = (Timer) m_TableStealth[ attacker ];
					...
					m_TableStealth[ attacker ] = t1 = Timer.DelayCall( TimeSpan.FromSeconds( 5.0 ), new TimerStateCallback( Expire_CallbackStealth ), attacker );

					defender.FixedParticles( 0x37B9, 1, 4, 0x13AE, 0, 0, EffectLayer.Waist );
				}
				else
				{
			...
			if ( Backstab.UnderEffect( attacker ) )
			{
				double distance = attacker.GetDistanceToSqrt( Backstab.CasterLocation( attacker ) );

				Spell spell = Backstab.GetSpell( attacker );

				if ( spell.CheckSequence() )
				{
					if ( Backstab.UnderEffect( attacker ) && !attacker.Hidden )
					{
						attacker.SendLocalizedMessage( 1063087 ); // You must be in stealth mode to use this ability.					

						attacker.RevealingAction();

						attacker.Send( new SetNewSpell( 0xFA, 0 ) );

						Backstab.m_Table.Remove( attacker );

						Backstab.m_Table2.Remove( attacker );

						Backstab.m_Table3.Remove( attacker );
					...
					int damage_bonus = (int) (attacker.Skills[ SkillName.Ninjitsu ].Value/3.0 + distance*2.0); // TODO: verify
					...
					Timer backstab = (Timer) m_TableStealth[ attacker ];
					...
					m_TableStealth[ attacker ] = backstab = Timer.DelayCall( TimeSpan.FromSeconds( 5.0 ), new TimerStateCallback( Expire_CallbackStealth ), attacker );
				}
			...
			if ( KiAttack.UnderEffect( attacker ) )
			{
				double distance = attacker.GetDistanceToSqrt( KiAttack.CasterLocation( attacker ) );

				Spell spell = KiAttack.GetSpell( attacker );
				...
				if ( spell.CheckSequence() )
				{
					if ( distance > 1 )
					{
						...
						int damage_bonus = (int) (distance*2);
				...
				attacker.Send( new SetNewSpell( 0xF8, 0 ) );

				KiAttack.m_Table.Remove( attacker );

				KiAttack.m_Table2.Remove( attacker );

				KiAttack.m_Table3.Remove( attacker );

				spell.FinishSequence();
			}

			if ( FocusAttack.UnderEffect( attacker ) )
			{
				Spell spell = FocusAttack.GetSpell( attacker );
				...
				attacker.Send( new SetNewSpell( 0xF5, 0 ) );

				FocusAttack.m_Table.Remove( attacker );

				FocusAttack.RemoveBonus( this );

				FocusAttack.m_Table2.Remove( attacker );

				spell.FinishSequence();
			}

			if ( attacker.BodyMod == 0x15 && !(this is BaseRanged) )
			{
				defender.Poison = Poison.Lesser;
			}

			if ( defender.BodyMod == 0x51 && !(this is BaseRanged) )
			{
				attacker.Poison = Poison.Regular;
			}
			#endregion
			...
			if ( m_InDoubleStrike )
			{
				// 10% loss when attacking with double-strike
				damage -= AOS.Scale( damage, 10 );
			}
			...
			if ( m_Slayer2 == SlayerName.Silver && context != null && context.Type != typeof( HorrificBeastSpell ) )
			{
				// Every necromancer transformation other than horrific beast take an additional 25% damage
				damage += AOS.Scale( damage, 25 );
			} 
			...
			try
			{
				...
				{
					if ( m != null && (m is Clone && ((Clone) m).Owner == defender && ((Clone) m).Owner != attacker) )
					{
			...
			if ( damage > 0 )
			{
				if ( ParalyzingBlow.UnderEffect( defender ) )
				{
					defender.Frozen = false;
					ParalyzingBlow.m_Table.Remove( defender );
				}
			}
			...
			if ( Core.AOS )
			{
				...
				{
					// HitLeechHits% chance to leech 30% of damage as hit points
					lifeLeech += 30;
				} 
				...
				{
					// HitLeechMana% chance to leech 40% of damage as mana
					manaLeech += 40;
				} 
				...
				{
					// Vampiric embrace gives an additional 20% life leech
					lifeLeech += 20;
				} 

				if ( context != null && context.Type == typeof( WraithFormSpell ) )
				{
					int mana = (5 + (int) ((15*attacker.Skills.SpiritSpeak.Value)/100)); // Wraith form gives an additional 5-20% mana leech
					manaLeech += mana;
					defender.Mana -= AOS.Scale( damageGiven, mana );
				}
		...
		public virtual CheckSlayerResult CheckSlayers( Mobile attacker, Mobile defender )
		{
			...
			SlayerEntry defSlayer2 = SlayerGroup.GetEntryByName( defWeapon.Slayer2 );
			...
			if ( defSlayer2 != null && defSlayer2.Group.OppositionSuperSlays( attacker ) )
			{
				return CheckSlayerResult.Opposition;
			}
		...
		public virtual int GetDamageBonus()
		{
			...
			PlayerMobile pm = this.Parent as PlayerMobile;

			if ( pm != null && pm.HonorOpponent != null && pm.Skills[ SkillName.Bushido ].Base >= 50 )
			{
				bonus += (int) ((0.1*pm.Perfection)*bonus);
			}

			if ( pm != null && HonorVirtue.UnderHonorEmbrace( pm ) )
			{
				bonus += (int) (0.25*bonus);
			}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 7:
					{
						m_FocusCoeff = reader.ReadDouble();
			...
			if ( Slayer == SlayerName.DaemonDismissal || Slayer == SlayerName.BalronDamnation )
			{
				Slayer = SlayerName.Exorcism;
			}

			if ( Slayer2 == SlayerName.DaemonDismissal || Slayer == SlayerName.BalronDamnation )
			{
		...
		public static BaseWeapon Fists { get { return m_Fists; } set { m_Fists = value; } }
