...
namespace Server.Items
{
	...
	public abstract class BaseBeverage : Item, IHasQuantity
	{
		...
		private class ThrowTarget : Target
		{
			...
			public ThrowTarget( BaseBeverage bomb ) : base( 12, true, TargetFlags.None )
			{
				m_Bomb = bomb;
			}
		...
		public void Explode( Mobile from, bool firefield, Point3D loc, Map map )
		{
			...
			if ( firefield )
			{
				...
				new FireFieldSpell.InternalItem( 0x37C3, loc, from, map, TimeSpan.FromSeconds( 10.0 ), 0, 0 );

				new FireFieldSpell.InternalItem( 0x398C, loc, from, map, TimeSpan.FromSeconds( 10.0 ), 0, Utility.RandomMinMax( 2, 4 ) );
			}
			else
			{
