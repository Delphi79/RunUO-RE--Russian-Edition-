...
namespace Server.SkillHandlers
{
	public class AnimalTaming
	{
		...
		public static void Scale( BaseCreature bc, double scalar, bool scaleStats, bool scaleSkills )
		{
			if ( scaleStats )
			{
				if ( bc.RawStr > 0 )
					...
					else
					{
						...
						if ( alreadyOwned || m_Tamer.CheckTargetSkill( SkillName.AnimalTaming, m_Creature, minSkill - 25.0, minSkill + 25.0 ) )
						{
							if ( m_Creature.Owners.Count == 0 ) // First tame
							{
								if ( m_Paralyzed )
								{
									Scale( m_Creature, 0.86, false, true ); // 86% of original skills if they were paralyzed during the taming
								}
								else
								{
									Scale( m_Creature, 0.90, false, true ); // 90% of original skills
								}

								if ( m_Creature.SubdueBeforeTame )
								{
									Scale( m_Creature, 0.50, true, true ); // Creatures which must be subdued take an additional 50% loss of skills and stats
								}

								if ( m_Creature is Hiryu || m_Creature is LesserHiryu )
								{
									Scale( m_Creature, 0.5, true, false );
								}
