...
namespace Server.Mobiles
{
	public class SamuraiAI : BaseAI
	{
		...
		public SamuraiAI( BaseCreature m ) : base( m )
		{
		}
		...
		private void UseAttackAbility()
		{
			...
			if ( t.Count > 1 )
			{
				if ( Utility.Random( 3 ) != 1 )
				{
					...
					{
						new MomentumStrike( m_Mobile, null ).Cast();
						return;
					}
				}
			}
			...
			{
				new LightningStrike( m_Mobile, null ).Cast();
			}
		...
		{
			if ( Confidence.UnderEffect( m_Mobile ) || Evasion.UnderEffect( m_Mobile ) || CounterAttack.UnderEffect( m_Mobile ) )
			{
				return;
			}
		...
		public override bool DoActionCombat()
		{
			...
			if ( combatant.Hits < (Utility.Random( 10 ) + 10) )
			{
				...
				{
					new HonorableExecution( m_Mobile, null ).Cast();
					return true;
				}
			}
