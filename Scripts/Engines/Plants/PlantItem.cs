...
namespace Server.Engines.Plants
{
	...
	public class PlantItem : Item, ISecurable
	{
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool IsGrowable { get { return m_PlantStatus >= PlantStatus.BowlOfDirt && m_PlantStatus <= PlantStatus.Stage9; } }

		[CommandProperty( AccessLevel.GameMaster )]
		public bool IsCrossable { get { return PlantHueInfo.IsCrossable( this.PlantHue ) && PlantTypeInfo.IsCrossable( this.PlantType ); } }
		...
		public override void AddNameProperty( ObjectPropertyList list )
		{
			if ( m_PlantStatus >= PlantStatus.DecorativePlant )
			{
				...
				list.Add( 1070973, "#{0}\t#{1}", title, typeInfo.Name ); // a decorative ~1_COLOR~ ~2_TYPE~ plant
			}
			else if ( m_PlantStatus >= PlantStatus.FullGrownPlant )
			{
				PlantHueInfo hueInfo = PlantHueInfo.GetInfo( m_PlantHue );
