...
namespace Server.Engines.CannedEvil
{
	public class ChampionSpawn : Item
	{
		...
		public bool IsValorUsed { get { return m_IsValorUsed; } set { m_IsValorUsed = value; } }

		[CommandProperty( AccessLevel.GameMaster )]
		public ChampionIdol Idol { get { return m_Idol; } set { m_Idol = value; } }
		...
		public int MaxKills { get { return 250 - (Level*12); } }
		...
		public PlayerMobile GetPlayerFromMobileKiller( Mobile killer )
		{
			if ( killer == null )
			{
				return null;
			...
			{
				BaseCreature creature = killer as BaseCreature;

				if ( creature.Controled )
				{
					return GetPlayerFromMobileKiller( creature.ControlMaster );
				}
				else if ( creature.Summoned )
				{
					return GetPlayerFromMobileKiller( creature.SummonMaster );
				}
				else
				{
					return null;
		...
		public void Respawn()
		{
			...
			while ( m_Creatures.Count < (50 - (GetSubLevel()*10)) )
			{
				...
				if ( m is BaseCreature )
				{
					...
					((BaseCreature) m).SpawnLevel = value;

					((BaseCreature) m).ChampionType = Type;
				}
			}
		}
		...
		private static Type[][][] m_Types = new Type[7][][]
			{
				new Type[][]
				{																											// Abyss
