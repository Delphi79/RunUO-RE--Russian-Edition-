...
namespace Server.Gumps
{
	public class ChooseMinorArtifactGump : Gump
	{
		...
		public ChooseMinorArtifactGump( Mobile m, IharaSoko min, ArrayList l ) : base( 0, 0 )
		{
			from = m;
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID > 0 )
			{
				...
				if ( item != null && from.Backpack.ConsumeTotal( item.GetType(), 1 ) )
				{
					if ( pm.GivenArtifacts < 10 )
					{
						...
						if ( pm.GivenArtifacts != 10 )
						{
							string args = String.Format( "{0}\t{1}\t ", pm.GivenArtifacts.ToString(), (10 - pm.GivenArtifacts).ToString() );
				...
				if ( list.Count != 0 && pm.GivenArtifacts != 10 )
				{
