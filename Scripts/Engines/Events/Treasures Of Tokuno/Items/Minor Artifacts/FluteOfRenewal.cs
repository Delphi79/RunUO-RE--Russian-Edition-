...
namespace Server.Items
{
	public class FluteOfRenewal : BambooFlute
	{
		...
		private static int[] m_SuperSlayers = new int[]
			{
				(int)SlayerName.Exorcism,
				(int)SlayerName.Silver,
				(int)SlayerName.Repond,
				(int)SlayerName.ArachnidDoom,
				(int)SlayerName.ReptilianDeath
			};
		...
		private class ReplenishTimer : Timer
		{
			...
			public ReplenishTimer( FluteOfRenewal flute ) : base( TimeSpan.FromMinutes( 5.0 ) )
			{
				m_flute = flute;
			}
