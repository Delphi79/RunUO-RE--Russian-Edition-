...
namespace Server.Engines.Chat
{
	public class ChatActionHandlers
	{
		...
		public static void EmoteMessage( ChatUser from, Channel channel, string param )
		{
			...
			{
				// %1 %2
				channel.SendIgnorableMessage( 58, from, from.GetColorCharacter() + from.Username, param );
			} 
			else
			{
				// The moderator of this conference has not given you speaking priviledges.
		...
		public static void QueryWhoIs( ChatUser from, Channel channel, string param )
		{
			...
			if ( target.Anonymous )
			{
				...
				from.SendMessage( 41, target.Username );
			} 
			else
			{
				// %2 is known in the lands of Britannia as %2.
				from.SendMessage( 43, target.Username, target.Mobile.Name );
			} 
		}
