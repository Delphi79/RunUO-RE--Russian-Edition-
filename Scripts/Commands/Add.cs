				...
				if ( packs != null )
				{
					for ( int i = 0; i < packs.Count; ++i )
					{
						...
						if ( built is Item )
						{
							...
							if ( Miscellanious.ValidateLabeling( built ) )
							{
								((Item) built).Cheater_Name = String.Format( "This item created by GM {0}", from.Name );
							}
						...
						else if ( built is Mobile )
						{
							...
							if ( Miscellanious.ValidateLabeling( built ) )
							{
								m.Cheater_Name = String.Format( "This mobile created by GM {0}", from.Name );
							}
					...
					for ( int x = start.X; x <= end.X; ++x )
					{
						for ( int y = start.Y; y <= end.Y; ++y )
						{
							...
							if ( built is Item )
							{
								...
								if ( Miscellanious.ValidateLabeling( item ) )
								{
									item.Cheater_Name = String.Format( "This item created by GM {0}", from.Name );
								}
							...
							else if ( built is Mobile )
							{
								...
								if ( Miscellanious.ValidateLabeling( m ) )
								{
									m.Cheater_Name = String.Format( "This mobile created by GM {0}", from.Name );
								}
