...
namespace Server.Misc
{
	public class CharacterCreation
	{
		...
		private static void FillBankAOS( Mobile m )
		{
			...
			PlaceItemIn( cont, 45, 107, new Spellbook( UInt64.MaxValue ) );
			PlaceItemIn( cont, 64, 107, new NecromancerSpellbook( (UInt64) 0xFFFF ) );
			PlaceItemIn( cont, 83, 107, new BookOfChivalry( (UInt64) 0x3FF ) );

			if ( Core.SE )
			{
				PlaceItemIn( cont, 102, 107, new BookOfBushido() ); //Default ctor = full
				PlaceItemIn( cont, 121, 107, new BookOfNinjitsu() ); //Default ctor = full
			}

			Runebook runebook = new Runebook( 20 );
			runebook.CurCharges = runebook.MaxCharges;
			PlaceItemIn( cont, 141, 107, runebook );

			Item toHue = new BagOfReagents( 150 );
			toHue.Hue = 0x2D;
			PlaceItemIn( cont, 45, 149, toHue );

			toHue = new BagOfNecroReagents( 150 );
			toHue.Hue = 0x488;
			PlaceItemIn( cont, 64, 149, toHue );

			PlaceItemIn( cont, 141, 149, new BagOfAllReagents( 500 ) );

			for ( int i = 0; i < 9; ++i )
			{
				PlaceItemIn( cont, 45 + (i*10), 74, new RecallRune() );
			}
			...
			PlaceItemIn( cont, 129, 96, bracelet );

			PlaceItemIn( bank, 68, 169, cont );
			...
			for ( int i = 0; i < 10; i++ )
			{
				PlaceItemIn( cont, 117, 128, new MessageInABottle( Utility.RandomBool() ? Map.Trammel : Map.Felucca ) );
			}
			...
			PlaceItemIn( cont, 30, 60, new PeasantsBokuto() );
			PlaceItemIn( cont, 57, 60, new DragonNunchaku() );
			PlaceItemIn( cont, 38, 57, new TheDestroyer() );
			PlaceItemIn( cont, 36, 39, new HanzosBow() );
			PlaceItemIn( cont, 43, 83, new Exiler() );
			PlaceItemIn( cont, 92, 78, new PilferedDancerFans() );
			PlaceItemIn( cont, 45, 76, new DemonForks() );
			PlaceItemIn( cont, 49, 54, new BlackLotusHood() );
			PlaceItemIn( cont, 77, 79, new DaimyosHelm() );
			PlaceItemIn( cont, 39, 53, new ArmsOfTacticalExcellence() );
			PlaceItemIn( cont, 56, 80, new AncientFarmersKasa() );
			PlaceItemIn( cont, 69, 61, new GlovesOfTheSun() );
			PlaceItemIn( cont, 33, 44, new LegsOfStability() );
			PlaceItemIn( cont, 73, 86, new AncientSamuraiDo() );
			PlaceItemIn( cont, 47, 39, new PigmentsOfTokuno() );
			PlaceItemIn( cont, 67, 86, new FluteOfRenewal() );
			PlaceItemIn( cont, 59, 66, new AncientUrn() );
			PlaceItemIn( cont, 32, 0, new HonorableSwords() );
			PlaceItemIn( cont, 42, 35, new ChestOfHeirlooms() );
			PlaceItemIn( cont, 73, 79, new TomeOfEnlightenment() );
			...
			Yumi yumi = new Yumi();
			yumi.Attributes.WeaponSpeed = 35;
			yumi.Attributes.WeaponDamage = 35;

			RepeatingCrossbow repeating_crossbow = new RepeatingCrossbow();
			repeating_crossbow.Attributes.WeaponSpeed = 35;
			repeating_crossbow.Attributes.WeaponDamage = 35;

			HeavyCrossbow heavy_crossbow = new HeavyCrossbow();
			heavy_crossbow.Attributes.WeaponSpeed = 35;
			heavy_crossbow.Attributes.WeaponDamage = 35;

			Crossbow crossbow = new Crossbow();
			crossbow.Attributes.WeaponSpeed = 35;
			crossbow.Attributes.WeaponDamage = 35;

			CompositeBow composite_bow = new CompositeBow();
			composite_bow.Attributes.WeaponSpeed = 35;
			composite_bow.Attributes.WeaponDamage = 35;

			Bow bow = new Bow();
			bow.Attributes.WeaponSpeed = 35;
			bow.Attributes.WeaponDamage = 35;

			PlaceItemIn( cont, 41, 39, yumi );
			PlaceItemIn( cont, 73, 65, repeating_crossbow );
			PlaceItemIn( cont, 91, 54, heavy_crossbow );
			PlaceItemIn( cont, 47, 47, crossbow );
			PlaceItemIn( cont, 45, 49, composite_bow );
			PlaceItemIn( cont, 60, 51, bow );
		...
		private static void AddHair( Mobile m, int itemID, int hue )
		{
			...
			switch ( itemID & 0x3FFF )
			{
				case 0x2044:
					...
					break;
				case 0x2045:
					item = new PageboyHair( hue );
					break;
				case 0x2046:
					item = new BunsHair( hue );
					break;
				case 0x2047:
					item = new Afro( hue );
					break;
				case 0x2048:
					item = new ReceedingHair( hue );
					break;
				case 0x2049:
					item = new TwoPigTails( hue );
					break;
				case 0x204A:
					item = new KrisnaHair( hue );
					break;
				case 0x203B:
					item = new ShortHair( hue );
					break;
				case 0x203C:
					item = new LongHair( hue );
					break;
				case 0x203D:
					item = new PonyTail( hue );
					break;
				default:
					return;
			}
		...
		private static void AddBeard( Mobile m, int itemID, int hue )
		{
			...
			{
				case 0x203E:
					item = new LongBeard( hue );
					break;
				case 0x203F:
					item = new ShortBeard( hue );
					break;
				case 0x2040:
					item = new Goatee( hue );
					break;
				case 0x2041:
					item = new Mustache( hue );
					break;
				case 0x204B:
					item = new MediumShortBeard( hue );
					break;
				case 0x204C:
					item = new MediumLongBeard( hue );
					break;
				case 0x204D:
					item = new Vandyke( hue );
					break;
				default:
					return;
			}
		...
		private static void SetSkills( Mobile m, SkillNameValue[] skills, int prof )
		{
			switch ( prof )
			{
				case 1: // Warrior
					{
						skills = new SkillNameValue[] {new SkillNameValue( SkillName.Anatomy, 30 ), new SkillNameValue( SkillName.Healing, 45 ), new SkillNameValue( SkillName.Swords, 35 ), new SkillNameValue( SkillName.Tactics, 50 )};

						break;
					}
				case 2: // Magician
					{
						skills = new SkillNameValue[] {new SkillNameValue( SkillName.EvalInt, 30 ), new SkillNameValue( SkillName.Wrestling, 30 ), new SkillNameValue( SkillName.Magery, 50 ), new SkillNameValue( SkillName.Meditation, 50 )};

						break;
					}
				case 3: // Blacksmith
					{
						skills = new SkillNameValue[] {new SkillNameValue( SkillName.Mining, 30 ), new SkillNameValue( SkillName.ArmsLore, 30 ), new SkillNameValue( SkillName.Blacksmith, 50 ), new SkillNameValue( SkillName.Tinkering, 50 )};

						break;
					}
				case 4: // Necromancer
					{
						skills = new SkillNameValue[] {new SkillNameValue( SkillName.Necromancy, 50 ), new SkillNameValue( SkillName.Focus, 30 ), new SkillNameValue( SkillName.SpiritSpeak, 30 ), new SkillNameValue( SkillName.Swords, 30 ), new SkillNameValue( SkillName.Tactics, 20 )};

						break;
					}
				case 5: // Paladin
					{
						skills = new SkillNameValue[] {new SkillNameValue( SkillName.Chivalry, 51 ), new SkillNameValue( SkillName.Swords, 49 ), new SkillNameValue( SkillName.Focus, 30 ), new SkillNameValue( SkillName.Tactics, 30 )};

						break;
					}
				case 6: //Samurai
					{
						skills = new SkillNameValue[] {new SkillNameValue( SkillName.Bushido, 50 ), new SkillNameValue( SkillName.Swords, 50 ), new SkillNameValue( SkillName.Anatomy, 30 ), new SkillNameValue( SkillName.Healing, 30 ),};
						break;
					}
				case 7: //Ninja
					{
						skills = new SkillNameValue[] {new SkillNameValue( SkillName.Ninjitsu, 50 ), new SkillNameValue( SkillName.Hiding, 50 ), new SkillNameValue( SkillName.Fencing, 30 ), new SkillNameValue( SkillName.Stealth, 30 ),};
						break;
					}
				default:
					{
