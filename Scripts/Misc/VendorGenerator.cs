...
namespace Server
{
	public class VendorGenerator
	{
		...
		private static Rectangle2D[] m_IlshRegions = new Rectangle2D[] {new Rectangle2D( new Point2D( 0, 0 ), new Point2D( 288*8, 200*8 ) )};
		...
		private static bool IsStaticFloor( int itemID )
		{
			...
			return (itemID >= 0x495 && itemID <= 0x514) || (itemID >= 0x519 && itemID <= 0x53A);
		}
		...
		private static bool IsDisplayCase( int itemID )
		{
			...
			return (itemID >= 0xB00 && itemID <= 0xB02) || (itemID >= 0xB06 && itemID <= 0xB0A) || (itemID >= 0xB0D && itemID <= 0xB17);
		}
