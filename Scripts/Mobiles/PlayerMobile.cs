...
namespace Server.Mobiles
{
	...
	public enum PlayerFlag // First 16 bits are reserved for default-distro use, start custom flags at 0x00010000
	{
		...
		Young = 0x00000400,
		ToggleMonstersTitles = 0x00000800,
		AllowGuildInvites = 0x00100000
	}
	...
	public class PlayerMobile : Mobile
	{
		...
		private int m_GivenArtifacts;
		private double m_ChanceGetArtifact;
		...
		private int m_DefendChance;
		private int m_SwingSpeed;
		private int m_DamageIncrease;
		private int m_LowerManaCost;

		public int HitChance { get { return m_HitChance; } set { m_HitChance = value;  InvalidateProperties(); } }
		public int DefendChance { get { return m_DefendChance; } set { m_DefendChance = value;  InvalidateProperties(); } }
		public int SwingSpeed { get { return m_SwingSpeed; } set { m_SwingSpeed = value;  InvalidateProperties(); } }
		public int DamageIncrease { get { return m_DamageIncrease; } set { m_DamageIncrease = value;  InvalidateProperties(); } }
		public int LowerManaCost { get { return m_LowerManaCost; } set { m_LowerManaCost = value;  InvalidateProperties(); } }

		public int GivenArtifacts { get { return m_GivenArtifacts; } set { m_GivenArtifacts = value; } }

		public double ChanceGetArtifact { get { return m_ChanceGetArtifact; } set { m_ChanceGetArtifact = value; } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool AllowGuildInvites { get { return GetFlag( PlayerFlag.AllowGuildInvites ); } set { SetFlag( PlayerFlag.AllowGuildInvites, value ); } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public bool ToggleMonstersTitles { get { return GetFlag( PlayerFlag.ToggleMonstersTitles ); } set { SetFlag( PlayerFlag.ToggleMonstersTitles, value ); } }
		...
		private static void AutoStable( Mobile from )
		{
			...
			foreach( Mobile m in World.Mobiles.Values )
			{
				...
				if ( creature != null && creature.Controled && creature.ControlMaster == from && 
		...
		private static void OnLogin( LoginEventArgs e )
		{
			...
			{
				PropertiesTimer timer = new PropertiesTimer( (from as PlayerMobile) );
				timer.Start();
			}
		...
		private static void EventSink_Disconnected( DisconnectedEventArgs e )
		{
			...
			PlayerMobile pml = e.Mobile as PlayerMobile;
			pml.LastLogin = DateTime.Now;

			// at OSI for logout from Lamp Room, account may be banned
			PlayerMobile m = e.Mobile as PlayerMobile;

			if ( m != null && m.Region.Name == "Doom Lamp Room" )
			{
		...
		public override bool AllowSkillUse( SkillName skill )
		{
			...
			if ( AnimalForm.UnderEffect( this ) )
			{
				...
				this.SendLocalizedMessage( 1070771 ); // 1070771 - You cannot use that skill in this form.
				return false;
			}
			return true;
		}
		...
		private bool CanInsure( Item item )
		{
			...
			if ( item is TomeOfEnlightenment || item is TomeOfLostKnowledge )
			{
				return true;
			}
			...
			if ( item.Parent != null && item.Parent is Container && ((Container) item.Parent).IsLockedContainer )
			{
				return false;
			}
		...
		public override void OnDamage( int amount, Mobile from, bool willKill )
		{
			...
			try
			{
				if ( from != null && from.Location == ((PlayerMobile) from).SpotHonor && ((PlayerMobile) from).HonorOpponent != null && ((PlayerMobile) from).HonorOpponent == this )
				{
					Item item = this.FindItemOnLayer( Layer.TwoHanded );

					if ( item != null && item is BaseRanged )
					{
						((PlayerMobile) from).IsHonorPenalty = true;

						return;
					}
				}

				if ( HonorOpponent != null && HonorOpponent != from )
				{
					IsHonorPenalty = true;
		...
		public override bool OnBeforeDeath()
		{
			...
			this.Send( new FastMovePacket( false ) );
			AnimalForm.m_Table.Remove( this );
		...
		public DateTime LastEscortTime { get { return m_LastEscortTime; } set { m_LastEscortTime = value; } }
		...
		public ArrayList VisibilityList { get { return m_VisList; } }

		public ArrayList PermaFlags { get { return m_PermaFlags; } }
		...
		public Engines.BulkOrders.BOBFilter BOBFilter { get { return m_BOBFilter; } }
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 22:
					{
						m_GivenArtifacts = reader.ReadInt();

						m_ChanceGetArtifact = reader.ReadDouble();
			...
			if ( this != null && this.Region.Name == "Doom Lamp Room" )
			{
				Rectangle2D rect = new Rectangle2D( 342, 168, 16, 16 );
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( (int) m_GivenArtifacts );

			writer.Write( (double) m_ChanceGetArtifact );
		...
		public override bool CanSee( Item item )
		{
			...
			// TODO: what happenes when player in one party with owner of item??
			if ( this.AccessLevel == AccessLevel.Player && item.Keeper != null && item.Keeper != this )
			{
				return false;
			}
		...
		public class PropertiesTimer : Timer
		{
			private PlayerMobile pm;

			public void GetValues()
			{
				ArrayList items = pm.Items;
				int hitchance = 0;
				int defensechance = 0;
				int swingspeed = 0;
				...
				for ( int i = 0; i < items.Count; ++i )
				{
					...
					if ( obj is BaseWeapon )
					{
						...
						if ( weapon != null )
						{
							hitchance += weapon.Attributes.AttackChance;
							defensechance += weapon.Attributes.DefendChance;
							swingspeed += weapon.Attributes.WeaponSpeed;
							damageincrease += weapon.Attributes.WeaponDamage;
							lowermanacost += weapon.Attributes.LowerManaCost;
						}
					}
					else if ( obj is BaseArmor )
					{
						...
						if ( armor != null )
						{
							hitchance += armor.Attributes.AttackChance;
							defensechance += armor.Attributes.DefendChance;
							swingspeed += armor.Attributes.WeaponSpeed;
							damageincrease += armor.Attributes.WeaponDamage;
							lowermanacost += armor.Attributes.LowerManaCost;
						}
					}
					else if ( obj is BaseJewel )
					{
						...
						if ( jewel != null )
						{
							hitchance += jewel.Attributes.AttackChance;
							defensechance += jewel.Attributes.DefendChance;
							swingspeed += jewel.Attributes.WeaponSpeed;
							damageincrease += jewel.Attributes.WeaponDamage;
							lowermanacost += jewel.Attributes.LowerManaCost;
						}
					}
					else if ( obj is BaseClothing )
					{
						BaseClothing clothing = obj as BaseClothing;

						if ( clothing != null )
						{
							hitchance += clothing.Attributes.AttackChance;
							defensechance += clothing.Attributes.DefendChance;
							swingspeed += clothing.Attributes.WeaponSpeed;
							damageincrease += clothing.Attributes.WeaponDamage;
							lowermanacost += clothing.Attributes.LowerManaCost;
						}
					}
					else if ( obj is Spellbook )
					{
						Spellbook spellbook = obj as Spellbook;

						if ( spellbook != null )
						{
							hitchance += spellbook.Attributes.AttackChance;
							defensechance += spellbook.Attributes.DefendChance;
							swingspeed += spellbook.Attributes.WeaponSpeed;
							damageincrease += spellbook.Attributes.WeaponDamage;
							lowermanacost += spellbook.Attributes.LowerManaCost;
						}
					}
				}

				pm.HitChance = hitchance;
				pm.DefendChance = defensechance;
				pm.SwingSpeed = swingspeed;
				pm.DamageIncrease = damageincrease;
				pm.LowerManaCost = lowermanacost;
			}

			public PropertiesTimer( PlayerMobile m ) : base( TimeSpan.FromSeconds( 1.0 ), TimeSpan.FromSeconds( 1.0 ) )
			{
				pm = m;
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( TestCenter.Enabled )
			{
				...
				list.Add( 1060415, HitChance.ToString() );
				list.Add( 1060408, DefendChance.ToString() );
				list.Add( 1060486, SwingSpeed.ToString() );
				list.Add( 1060401, DamageIncrease.ToString() );
				list.Add( 1060433, LowerManaCost.ToString() );
			}
		...
		public bool BedrollLogout { get { return m_BedrollLogout; } set { m_BedrollLogout = value; } }

		#region Factions
		private PlayerState m_FactionPlayerState;

		public PlayerState FactionPlayerState { get { return m_FactionPlayerState; } set { m_FactionPlayerState = value; } }
		#endregion
		...
		public ArrayList DoneQuests { get { return m_DoneQuests; } set { m_DoneQuests = value; } }
		...
		public virtual TimeSpan ComputeMovementSpeed( Direction dir )
		{
			...
			bool AnimalForm = (this.BodyMod == 0x84 || this.BodyMod == 0x7A || this.BodyMod == 0xF6 || this.BodyMod == 0x19 || this.BodyMod == 0xDC || this.BodyMod == 0xDA);
		...
		public bool WaitingForEnemy { get { return m_WaitingForEnemy; } set { m_WaitingForEnemy = value; } }
		...
		private Mobile m_HonorOpponent;
		private Point3D m_SpotHonor;
		private bool m_IsHonorPenalty;
		private int m_Perfection;

		public DateTime LastHonorLoss { get { return m_LastHonorLoss; } set { m_LastHonorLoss = value; } }
		public Mobile HonorOpponent { get { return m_HonorOpponent; } set { m_HonorOpponent = value; } }
		public Point3D SpotHonor { get { return m_SpotHonor; } set { m_SpotHonor = value; } }
		public bool IsHonorPenalty { get { return m_IsHonorPenalty; } set { m_IsHonorPenalty = value; } }
		public int Perfection { get { return m_Perfection; } set { m_Perfection = value; } }
		...
		public double ValorGain { get { return m_ValorGain; } set { m_ValorGain = value; } }
		#endregion
		...
		private static Point3D[] m_TrammelDeathDestinations = new Point3D[] {new Point3D( 1481, 1612, 20 ), new Point3D( 2708, 2153, 0 ), new Point3D( 2249, 1230, 0 ), new Point3D( 5197, 3994, 37 ), new Point3D( 1412, 3793, 0 ), new Point3D( 3688, 2232, 20 ), new Point3D( 2578, 604, 0 ), new Point3D( 4397, 1089, 0 ), new Point3D( 5741, 3218, -2 ), new Point3D( 2996, 3441, 15 ), new Point3D( 624, 2225, 0 ), new Point3D( 1916, 2814, 0 ), new Point3D( 2929, 854, 0 ), new Point3D( 545, 967, 0 ), new Point3D( 3665, 2587, 0 )};

		private static Point3D[] m_IlshenarDeathDestinations = new Point3D[] {new Point3D( 1216, 468, -13 ), new Point3D( 723, 1367, -60 ), new Point3D( 745, 725, -28 ), new Point3D( 281, 1017, 0 ), new Point3D( 986, 1011, -32 ), new Point3D( 1175, 1287, -30 ), new Point3D( 1533, 1341, -3 ), new Point3D( 529, 217, -44 ), new Point3D( 1722, 219, 96 )};

		private static Point3D[] m_MalasDeathDestinations = new Point3D[] {new Point3D( 2079, 1376, -70 ), new Point3D( 944, 519, -71 )};

		private static Point3D[] m_TokunoDeathDestinations = new Point3D[] {new Point3D( 1166, 801, 27 ), new Point3D( 782, 1228, 25 ), new Point3D( 268, 624, 15 )};
		...
		public double[] ChampionTiers = new double[7];
		public int[] SuperChampionTiers = new int[8];
