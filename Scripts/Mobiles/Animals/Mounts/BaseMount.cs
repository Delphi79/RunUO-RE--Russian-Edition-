...
namespace Server.Mobiles
{
	public abstract class BaseMount : BaseCreature, IMount
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				bool canAccess = (from.AccessLevel >= AccessLevel.GameMaster) || (Controled && ControlMaster == from) || (Summoned && SummonMaster == from);
				...
				if ( canAccess )
				{
					if ( this.Poisoned )
					{
						...
						PrivateOverheadMessage( Network.MessageType.Regular, 0x3B2, 1049692, from.NetState );
					}
					else
					{
						Rider = from;
